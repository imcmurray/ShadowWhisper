...
### STEP 1.5: CHECK FOR USER GUIDANCE

Users can send guidance messages through the UI while you work. **Always check for guidance at the start of each session and between features:**

```
# Check for pending user guidance messages
Use the guidance_check tool
```

If guidance exists:

1. Read each message carefully
2. Acknowledge with `guidance_acknowledge(guidance_id)` to mark as processed
3. Incorporate the guidance into your current work
4. **Urgent messages take priority** over normal feature work

User guidance may include:
- Corrections to your approach
- Clarifications about requirements
- Requests to prioritize specific work
- Bug reports or issues they've noticed
- Feedback on your recent changes
...
### STEP 7.5: CHECK FOR GUIDANCE BEFORE NEXT FEATURE

Before starting the next feature, check if the user has sent any guidance:

```
Use the guidance_check tool
```

Users may have noticed issues with your recent work, want to redirect your priorities, or have feedback. Always check and respond to guidance between features.
...
## PLAYWRIGHT DOCKER ARCHITECTURE (CRITICAL)

Playwright browser automation runs inside a Docker container with a separate network namespace.

### URL Rewriting Rule (MANDATORY)

When using Playwright browser tools, **ALWAYS** convert `localhost` URLs:

| User Says              | You Navigate To               |
|------------------------|-------------------------------|
| `localhost:3000`       | `http://172.17.0.1:3000`      |
| `http://localhost:8080`| `http://172.17.0.1:8080`      |
| `localhost:5173/app`   | `http://172.17.0.1:5173/app`  |

**Why?** The container's `localhost` refers to itself (empty), not the host machine. `172.17.0.1` is the Docker bridge gateway routing to the host.

### Browser Is Pre-Configured

- Browsers are already installed in the Docker container
- The browser launches automatically on first `browser_navigate` call
- Start testing immediately - no setup or installation required
- **DO NOT** attempt to install browsers or run Playwright setup commands
...
# 8. Check for user guidance messages
guidance_check

# 9. Acknowledge guidance was received
guidance_acknowledge with guidance_id={id}
```
